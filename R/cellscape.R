...
cellscape <- function(cnv_data = NULL,
  ...
  if (is.null(cnv_data) && is.null(mut_data) && is.null(mut_data_matrix)) {
    stop(
      "User must provide either copy number data (parameter cnv_data)",
      " or mutation data (parameter mut_data or mut_data_matrix)."
    )
  }
  if (!is.null(cnv_data) && (!is.null(mut_data) || !is.null(mut_data))) {
    stop(
      "User can only provide copy number (parameter cnv_data) OR targeted mutations",
  ...
  if (is.data.frame(clone_colours)) {
    ...
    if (!("clone_id" %in% colnames(clone_colours)) ||
      ...
      stop(
        "Node colour data frame must have the following column names: ",
        "\"clone_id\", \"colour\""
      )
    }
  }
  ...
  if (!missing(gtype_tree_edges)) {
    ...
    if (!missing(sc_annot)) {
      ...
      if (length(gtypes_missing_from_gtype_tree) > 0) {
        stop(
          "The following clone ID(s) are present in the single cell annotations data but ",
  ...
  if (missing(mut_data)) {
    ...
    if (!("single_cell_id" %in% colnames(cnv_data)) ||
      ...
      stop(
        "CNV data frame (parameter cnv_data) must have the following column names: ",
    ...
    if ((height - 45) < n_scs) { # - 45 for top bar height (30) and space between top bar and main view (15)
      stop(
        "The number of single cells (", n_scs, ") cannot exceed the plot height minus 45px (",
  ...
  if (missing(cnv_data)) {
    ...
    if (!("single_cell_id" %in% colnames(mut_data)) ||
      ...
      stop(
        "Targeted mutations data frame must have the following column names: ",
    ...
    if (!is.null(mut_order)) {
      ...
      if (length(sites_missing_from_mut_order) > 0) {
        stop(
          "The following mutation(s) are present in the targeted mutation data but ",
      ...
      if (length(sites_extra_in_mut_order) > 0) {
        if (show_warnings) {
          print(paste("WARNING: The following mutation(s) are present in the mutation order data but ",
            ...
            paste(sites_extra_in_mut_order, collapse = ", "), ". They will have no effect on the visualization.",
            sep = ""
          ))
        }
  ...
  if (!("source" %in% colnames(tree_edges)) ||
    ...
    stop(
      "Tree edges data frame must have the following column names: ",
      "\"source\", \"target\""
    )
  }
    ...
    if (length(negative_dists) > 0) {
      ...
      if (show_warnings) {
        print(paste("WARNING: Negative distances found in tree edges data frame. Any negative distance will be ",
          "converted to ", value_for_neg_dists, ".",
          sep = ""
        ))
      }
  ...
  tree_nodes_for_layout <- data.frame(
    sc_id = unique_nodes,
  ...
  tree_edges_for_layout <- data.frame(
    source = sapply(tree_edges$source, function(src) {
      return(which(tree_nodes_for_layout$sc_id == src) - 1)
    }),
    source_sc_id = tree_edges$source,
    target = sapply(tree_edges$target, function(trg) {
      return(which(tree_nodes_for_layout$sc_id == trg) - 1)
    }),
  ...
  if (length(sources) > 1) {
    stop(
      "Multiple roots detected in tree (", paste(sources, collapse = ", "),
  ...
  if (length(scs_missing_from_tree) > 0) {
    ...
    if (show_warnings) {
      print(paste("WARNING: The following single cell ID(s) are present in the ", data_type, " data but ",
        ...
        ". They will not be shown in the visualization.",
        sep = ""
      ))
    }
  }
  ...
  if (!is.null(sc_annot)) {
    ...
    if (!("single_cell_id" %in% colnames(sc_annot)) ||
      !("genotype" %in% colnames(sc_annot))) {
      stop(
        "Single cell group assignment data frame must have the following column names: ",
  ...
  if (!is.null(gtype_tree_edges) &&
    ...
    if (length(gtypes_missing_from_tree) > 0) {
      stop(
        "The following genotype(s) are present in the single cell annotations data but ",
    ...
    timescape_userParams <- processUserData(
      clonal_prev,
      gtype_tree_edges,
      mutations,
      clone_colours,
...
getChromBoxInfo <- function(chrom_bounds, n_bp_per_pixel) {
  chrom_boxes <- data.frame(
    chr = chrom_bounds$chrom,
...
getCNVHeatmapForEachSC <- function(cnv_data, chrom_bounds, n_bp_per_pixel) {
  ...
  consecutive_px_merged <- dplyr::summarise(all_pixels_grouped,
    sum_px_width = sum(px_width),
    chr = chr[1],
    px_min = min(px),
    mode_cnv = mode_cnv[1]
  )
...
getTargetedHeatmapForEachSC <- function(mut_data, mut_order, heatmapWidth) {
  ...
  if (heatmapWidth < n_sites) {
    stop(
      "The number of mutation sites (", n_sites, ") cannot exceed the width of the plot (",
...
checkClonalPrev <- function(clonal_prev) {
  ...
  if (!("timepoint" %in% colnames(clonal_prev)) ||
    ...
    stop(
      "Clonal prevalence data frame must have the following column names: ",
...
getMutationsData <- function(mutations, tree_edges, clonal_prev) {
  if (is.data.frame(mutations)) {
    ...
    if (!("chrom" %in% colnames(mutations)) ||
      ...
      stop(
        "Mutations data frame must have the following column names: ",
    ...
    if (length(clone_ids_missing_from_tree_edges_data) > 0) {
      stop(
        "The following clone ID(s) are present in the mutations data but ",
        "are missing from the tree edges data: ",
    ...
    if (length(tps_missing_from_clonal_prev_data) > 0) {
      stop(
        "The following timepoint(s) are present in the mutations data but ",
    ...
    if (nrow(num_23) > 0) {
      stop(
        "Chromosome numbered \"23\" was detected in mutations data frame - X and Y chromosomes ",
